...
const FloatingDockDesktop = ({
  ...
  return (
    <motion.div
      ...
      className={cn(
        " h-16 items-end gap-4 rounded-2xl w-fit translate-y-18 px-4 pb-3 flex bg-neutral-950",
        className,
      )}
    >
      {items.map((item) => (
        ...
        <AnimatePresence>
          {hovered && (
            <motion.div
              ...
              className="absolute -top-8 left-1/2 w-fit rounded-md border  px-2 py-0.5 text-xs whitespace-pre text-neutral-700border-neutral-900 bg-neutral-800 text-white"
            >
              {title}
            </motion.div>
          )}
